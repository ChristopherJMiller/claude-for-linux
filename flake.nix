        ...
        let
          ...
          claudeApp = pkgs.stdenv.mkDerivation {
            ...
            buildPhase = ''
              ...
              perl -i -pe 's{([\w\x24]+=process\.platform==="darwin",)([\w\x24]+)(=process\.platform==="win32")}{$1$2$3||process.platform==="linux"}g' "$INDEX"
              grep -qP '[\w\x24]+=process\.platform==="win32"\|\|process\.platform==="linux"' "$INDEX" \
              ...
              perl -i -pe 's{(function )([\w\x24]+)(\(\)\{)(const t=process\.platform;if\(t!=="darwin"&&t!=="win32"\)return\{status:"unsupported")}{$1$2$3if(process.platform==="linux"\&\&global.__linuxCowork)return\{status:"supported"\};$4}g' "$INDEX"
              ...
              perl -i -pe 's{(async function [\w\x24]+\(t,e\)\{)(.{0,200}?\[downloadVM\])}{$1if(process.platform==="linux"\&\&global.__linuxCowork){console.log("[Cowork Linux] Skipping bundle download");return!1}$2}g' "$INDEX"
              grep -qP 'async function [\w\x24]+\(t,e\)\{if\(process\.platform==="linux"' "$INDEX" \
              ...
              perl -i -pe 's{(async function )([\w\x24]+)(\(\)\{)(const [\w\x24]+=await [\w\x24]+\(\);return\([\w\x24]+==null\?void 0:[\w\x24]+\.vm\)\?\?null)}{$1$2$3if(process.platform==="linux"\&\&global.__linuxCowork\&\&global.__linuxCowork.vmInstance){console.log("[Cowork Linux] $2() returning Linux VM");return global.__linuxCowork.vmInstance}$4}g' "$INDEX"
              grep -qP '\[Cowork Linux\] [\w\x24]+\(\) returning Linux VM' "$INDEX" \
              ...
              perl -i -pe 's{(async function [\w\x24]+\(\)\{return )process\.platform!=="darwin"\?null(:await [\w\x24]+\(\))}{''${1}process.platform!=="darwin"\&\&process.platform!=="linux"?null''${2}}g' "$INDEX"
              ...
              perl -i -pe 's{function ([\w\x24]+)\(\)\{return ([\w\x24]+)\.app\.isPackaged\?([\w\x24]+)\.resourcesPath:([\w\x24]+)\.resolve\(__dirname,"\.\.","\.\.","resources"\)\}}{function $1(){return process.platform==="linux"?$4.join($4.dirname($2.app.getAppPath()),"resources"):$2.app.isPackaged?$3.resourcesPath:$4.resolve(__dirname,"..","..","resources")}}g' "$INDEX"
              grep -qP 'process\.platform==="linux"\?[\w\x24]+\.join\([\w\x24]+\.dirname\(' "$INDEX" \
              ...
              perl -i -pe 's{([\w\x24]+)\?([\w\x24]+)=([\w\x24]+)\.nativeTheme\.shouldUseDarkColors\?"Tray-Win32-Dark\.ico":"Tray-Win32\.ico":([\w\x24]+)="TrayIconTemplate\.png"}{process.platform==="linux"?($2=$3.nativeTheme.shouldUseDarkColors?"TrayIconTemplate-Dark.png":"TrayIconTemplate.png"):$1?$2=$3.nativeTheme.shouldUseDarkColors?"Tray-Win32-Dark.ico":"Tray-Win32.ico":$4="TrayIconTemplate.png"}g' "$INDEX"
              ...
              perl -i -pe 's{([\w\x24]+)&&\(\1\.destroy\(\),\1=null\)}{$1&&($1.destroy(),$1=null,await new Promise(r=>setTimeout(r,250)))}g' "$INDEX"
              echo "[patch:09] Done"
